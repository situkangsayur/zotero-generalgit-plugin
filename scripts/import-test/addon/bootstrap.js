 ...
 * ZGIT_IMPORT_TEST_DIR is set it:
 *
 *   1. reads the repository from ZGIT_REMOTE_URL, ZGIT_BRANCH and ZGIT_BASE_PATH,
 *   2. runs Import from Git exactly as the menu item does, minus the prompt,
 *   3. writes report.json with what the library holds afterwards,
 *   4. quits Zotero.
 */

var log = msg => Zotero.debug(`[ZGIT import test] ${msg}`);
...
async function startup() {
	...
	let outDir = Services.env.get('ZGIT_IMPORT_TEST_DIR');
	if (!outDir) {
		return;
	}
...
async function run() {
	...
	plugin.Prefs.set('branch', env('ZGIT_BRANCH') || 'main');
