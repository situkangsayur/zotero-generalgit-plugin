...
Open an issue at https://github.com/situkangsayur/zotero-generalgit-plugin/issues. What helps most:
...
```bash
git clone https://github.com/situkangsayur/zotero-generalgit-plugin.git
cd zotero-generalgit-plugin
npm install          # only eslint; the plugin itself has no dependencies
npm test              # pure logic, no Zotero and no network
npm run test:git      # drives the real git binary against a throwaway repository
npm run build        # writes build/zotero-git-sync-<version>.xpi
```

Install `build/zotero-git-sync-<version>.xpi` in Zotero with Tools -> Add-ons -> the gear -> Install Add-on From File.
