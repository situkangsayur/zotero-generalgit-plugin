/* Zotero Git Sync -- the review panel shown before a sync decides for you
 *
...
ZoteroGitSync.Review = {
	...
	_build(win, review, resolve) {
		...
		let overlay = h('div', { id: 'zotero-git-sync-review', class: 'zgit-review-overlay', role: 'dialog', 'aria-modal': 'true' });
		let finish = (value) => {
			if (settled) {
				return;
			}
		...
		let summary = h('div', { class: 'zgit-review-summary' });
		let updateSummary = () => {
		...
		let rowTitle = row => h('div', { class: 'zgit-review-text' },
			h('div', { class: 'zgit-review-title', text: row.title || row.path }),
			h('div', { class: 'zgit-review-detail', text: [row.detail, row.path].filter(Boolean).join(' · ') }));
		...
		let checkSection = (id, rows, { heading, description, label, defaultValue = true, rowDetail = null }) => {
			...
			let boxes = [];
			let list = h('div', { class: 'zgit-review-list' });
			for (let row of rows) {
				...
				list.append(h('label', { class: 'zgit-review-row' }, box, rowTitle(rowDetail ? { ...row, detail: rowDetail(row) } : row)));
			}
			let setAll = (value) => {
			...
			return h('section', { class: 'zgit-review-section' },
				h('div', { class: 'zgit-review-section-head' },
					h('h2', { text: `${heading} (${rows.length})` }),
					h('span', { class: 'zgit-review-bulk' },
				...
				h('p', { class: 'zgit-review-description', text: description }),
				list);
		};
		...
		let conflictSection = () => {
			...
			rows.forEach((row) => {
				...
				let options = h('div', { class: 'zgit-review-segments', role: 'group' }, buttons);
				list.append(h('div', { class: 'zgit-review-row zgit-review-conflict' },
					rowTitle(row),
					h('div', { class: 'zgit-review-choices' }, options)));
			});
			return h('section', { class: 'zgit-review-section' },
				h('div', { class: 'zgit-review-section-head' }, h('h2', { text: `${get('review.conflicts')} (${rows.length})` })),
				h('p', { class: 'zgit-review-description', text: get('review.conflictsHelp') }),
				list);
		};
		...
		if (!sections.length) {
			sections.push(h('p', { class: 'zgit-review-empty', text: get('review.nothing') }));
		}

		let counts = review.counts;
		let card = h('div', { class: 'zgit-review-card' },
			h('header', { class: 'zgit-review-header' },
			...
			h('div', { class: 'zgit-review-body' }, sections),
			h('footer', { class: 'zgit-review-footer' },
				summary,
				h('button', { type: 'button', class: 'zgit-review-cancel', text: get('review.cancel'), onclick: () => finish(null) }),
				h('button', { type: 'button', class: 'zgit-review-confirm', text: get('review.confirm'), onclick: () => finish(this._decisions(review, checks, choices)) })));
		...
		updateSummary();
		card.querySelector('.zgit-review-confirm')?.focus();
