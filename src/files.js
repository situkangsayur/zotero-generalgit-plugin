/* Zotero Git Sync -- attachment files on disk
 *
...
ZoteroGitSync.Files = {
	...
	async hashFile(path, kind) {
		...
		if (kind !== 'lfs') {
			let header = Array.from(ZoteroGitSync.Utils.encode(`blob ${size}\0`));
	...
	_update(hash, chunk) {
		if (this._typedArrayUpdate) {
			try {
				...
				ZoteroGitSync.log(`nsICryptoHash.update() rejected a Uint8Array; using plain arrays (${e})`);
	...
	get cachePath() {
		return PathUtils.join(Zotero.Profile.dir, 'zotero-git-sync', 'hash-cache.json');
	},
