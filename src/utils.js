/* Zotero Git Sync -- small helpers with no Zotero or Git knowledge */
...
ZoteroGitSync.Utils = {
	...
	/**
	 * Two-character directory for a Zotero key. Git hosts' web interfaces slow
	 * down on directories with thousands of entries; Zotero keys draw on 32
	 * characters, so this spreads a library over at most 1,024 directories.
	 *
	 * @param {String} key
	 * @return {String}
	 */
	...
	/**
	 * Split a Git remote URL into its parts. Understands URLs (ssh://, https://,
	 * git://, file://) and scp-like SSH addresses (git@host:owner/repo.git).
	 *
	 * @param {String} url
	 * @return {{scheme: String, host: String, port: String, path: String}|null}
	 * 		null for local paths
	 */
	parseRemoteURL(url) {
		let str = String(url || '').trim();
		let match = str.match(/^([a-z][a-z0-9+.-]*):\/\/(?:[^@/]*@)?(\[[^\]]+\]|[^/:]*)(?::(\d*))?(\/.*)?$/i);
		if (match) {
			return {
		...
		// scp-like syntax: no scheme, a colon before the first slash
		match = str.match(/^(?:[^@/]+@)?([^/:]+):(?!\/\/)(.+)$/);
		if (match && !/^[a-z]$/i.test(match[1])) {
			return { scheme: 'ssh', host: match[1], port: '', path: match[2].replace(/^\/+/, '') };
		}
		return null;
	},
	...
	/**
	 * @param {String} url
	 * @return {String} e.g. "gitlab.com/owner/repo", with no credentials, or the
	 * 		last path segments of a local repository
	 */
	remoteLabel(url) {
		let parts = this.parseRemoteURL(url);
		if (!parts || parts.scheme === 'file') {
			let segments = String(parts ? parts.path : url || '').split(/[\\/]+/).filter(Boolean);
			return segments.slice(-2).join('/').replace(/\.git$/, '');
		}
		return `${parts.host}/${parts.path.replace(/\/+$/, '').replace(/\.git$/, '')}`;
	},


	/**
	 * Guess the repository's web page from its remote URL. Right for GitLab,
	 * Gitea, Forgejo, Bitbucket and GitHub, which all serve the repository at
	 * https://<host>/<path>; wrong for servers with no web interface, which is
	 * why the preference can override it.
	 *
	 * @param {String} url
	 * @return {String} '' when there is nothing sensible to open
	 */
	remoteWebURL(url) {
		let parts = this.parseRemoteURL(url);
		if (!parts || !['ssh', 'git', 'http', 'https'].includes(parts.scheme) || !parts.host) {
			return '';
		}
		let path = parts.path.replace(/\/+$/, '').replace(/\.git$/, '');
		if (!path || path.startsWith('~')) {
			return '';
		}
		// A custom HTTP(S) port is where the web interface lives too; an SSH port
		// says nothing about it
		let port = (parts.scheme === 'http' || parts.scheme === 'https') && parts.port ? `:${parts.port}` : '';
		let scheme = parts.scheme === 'http' ? 'http' : 'https';
		return `${scheme}://${parts.host}${port}/${path}`;
	},


	/**
	 * @param {String} text - Anything that may contain a URL with a password in it
	 * @return {String} The same text with user:password@ removed from URLs
	 */
	redactURLs(text) {
 ...
 */
ZoteroGitSync.CancelledError = class CancelledError extends Error {
...
/**
 * A cancellation flag that long waits and running git processes listen to, so
 * pressing Cancel takes effect immediately rather than after a push finishes.
 */
ZoteroGitSync.CancelToken = class CancelToken {
	constructor() {
	...
	/**
	 * @param {Function} listener - Called once, when the token is cancelled
	...
	sleep(ms) {
		...
		return new Promise((resolve, reject) => {
			let onCancel = () => {
				clearTimeout(timer);
				reject(new ZoteroGitSync.CancelledError());
			};
