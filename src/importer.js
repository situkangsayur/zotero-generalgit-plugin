/* Zotero Git Sync -- read a synced repository back into Zotero
 *
...
ZoteroGitSync.Importer = {
	// Item records read per `git cat-file --batch`
	READ_BATCH: 500,
	...
	async run({ config, token, cancel = null, onProgress = () => {} }) {
		let repo = await ZoteroGitSync.Sync.openRepository(config, { token, cancel });
		onProgress(ZoteroGitSync.getString('progress.fetching', ZoteroGitSync.Prefs.getRepoLabel(config)));
		let head = await repo.fetch();
		if (!head) {
			throw new Error(
				`${ZoteroGitSync.Prefs.getRepoLabel(config)} has no branch "${config.branch}" to import.`
			);
		}
		let remoteFiles = await repo.listTree(head);
		...
		return this._importGroups({ repo, config, libraries, cancel, onProgress });
	},


	/**
	 * Import only the repository paths a person accepted in the review: changes
	 * made in the repository or by another computer, and conflicts resolved in favour of
	 ...
	 *
	 * @param {Object} options
	 * @param {ZoteroGitSync.Repository} options.repo - Already fetched
	 ...
	 */
	async applyIncoming({ repo, config, remoteFiles, relPaths, keepBoth = new Set(), cancel = null, onProgress = () => {} }) {
		...
		return this._importGroups({
			repo, config, libraries, cancel, onProgress,
			force: true,
			overwriteFiles: true,
			keepBoth,
		});
	...
	async _importGroups({ repo, config, libraries, cancel, onProgress, force = false, overwriteFiles = false, keepBoth = new Set() }) {
		let totals = { status: 'ok', created: 0, updated: 0, skipped: 0, files: 0, failures: [] };
		let ctx = { repo, config, totals, onProgress, cancel, lfsAvailable: null };

		for (let [dir, group] of libraries) {
			cancel?.throwIfCancelled();
			let libraryID = this._resolveLibraryID(dir);
			if (libraryID === null) {
				ZoteroGitSync.log(`Skipping ${dir}: no matching local library`);
				totals.skipped += group.items.length;
				continue;
			}
			if (!Zotero.Libraries.get(libraryID)?.editable) {
				ZoteroGitSync.log(`Skipping ${dir}: library is read-only`);
				totals.skipped += group.items.length;
				continue;
			}
			...
			if (group.collections) {
				await this._importCollections({ repo, entry: group.collections, libraryID });
			}

			if (group.items.length) {
				let records = await this._fetchItemRecords({ repo, entries: group.items, onProgress, cancel });
			...
			if ((config.includeAttachments || overwriteFiles) && group.files.size) {
				onProgress(ZoteroGitSync.getString('progress.restoring'));
				totals.files += await this._restoreFiles({ ctx, group, libraryID, overwrite: overwriteFiles, keepBoth });
			}

			if (group.searches) {
				await this._importSearches({ repo, entry: group.searches, libraryID });
			}
			if (group.settings) {
				await this._importSettings({ repo, entry: group.settings, libraryID });
			}
		}

		if (totals.failures.length) {
			totals.failures.forEach(f => ZoteroGitSync.warn(f));
		}
		return totals;
	},
	...
	async _readJSON(repo, sha) {
		return JSON.parse(await repo.getBlobText(sha));
	},


	async _importCollections({ repo, entry, libraryID }) {
		let collections;
		try {
			collections = await this._readJSON(repo, entry.sha);
		}
		catch (e) {
	...
	async _fetchItemRecords({ repo, entries, onProgress, cancel = null }) {
		let records = [];
		for (let i = 0; i < entries.length; i += this.READ_BATCH) {
			cancel?.throwIfCancelled();
			let chunk = entries.slice(i, i + this.READ_BATCH);
			let blobs = await repo.readBlobs(chunk.map(entry => entry.sha));
			for (let entry of chunk) {
				try {
					records.push({ key: entry.key, data: JSON.parse(ZoteroGitSync.Utils.textDecoder.decode(blobs.get(entry.sha))) });
				}
				catch (e) {
					ZoteroGitSync.logError(new Error(`Could not read item ${entry.key}: ${e.message || e}`));
				}
			}
			onProgress(`Read ${Math.min(i + this.READ_BATCH, entries.length)}/${entries.length} items…`);
		}
		return records;
	},
	...
	async _restoreFiles({ ctx, group, libraryID, overwrite = false, keepBoth = new Set() }) {
		let { repo, config, totals, cancel } = ctx;
		let prefix = config.basePath ? `${config.basePath}/` : '';
		let written = 0;
		let copies = [];
			...
			for (let entry of entries) {
				...
				try {
					...
					if (entry.lfs) {
						...
						pointerText = await repo.getBlobText(entry.sha);
					...
					if (!both) {
						...
						if (stat && !overwrite) {
							...
							totals.failures.push(
								...
								+ '(choose it in Review Changes and Sync to replace it)'
							);
							continue;
						}
					}
					if (pointerText) {
						if (ctx.lfsAvailable === null) {
							ctx.lfsAvailable = await repo.lfsAvailable();
						}
						if (!ctx.lfsAvailable) {
							totals.failures.push(`${label}: stored with Git LFS, which is not installed on this computer`);
							continue;
						}
						await repo.lfsDownload(pointerText, target, prefix + entry.path);
					}
					else {
		...
		for (let copy of copies) {
			try {
				...
				let renamed = PathUtils.join(PathUtils.parent(copy.target), `${base} (repository copy)${ext}`);
				...
				let imported = await Zotero.Attachments.importFromFile({
					...
					title: copies.filter(c => c.attachment.id === copy.attachment.id).length > 1
						? `${copy.attachment.getField('title') || base}: ${copy.name} (repository copy)`
						: `${copy.attachment.getField('title') || base} (repository copy)`,
				});
				if (imported) {
					written++;
				}
	...
	async _importSearches({ repo, entry, libraryID }) {
		let searches;
		try {
			searches = await this._readJSON(repo, entry.sha);
		}
		catch (e) {
			ZoteroGitSync.logError(e);
			return;
		...
		for (let json of Array.isArray(searches) ? searches : []) {
			...
			try {
				...
				ZoteroGitSync.logError(new Error(`Failed to import saved search ${json.key}: ${e.message || e}`));
			}
		}
	},


	async _importSettings({ repo, entry, libraryID }) {
		let settings;
		try {
			settings = await this._readJSON(repo, entry.sha);
		}
		catch (e) {
			ZoteroGitSync.logError(e);
			return;
