/* Zotero Git Sync -- turns a Zotero library into a set of repository files
 *
...
ZoteroGitSync.Exporter = {
	...
	async build({ config, items = null, onProgress = () => {} }) {
		...
		if (!items) {
			out.files.set('.zotero-sync/manifest.json', this._jsonFile({
				...
				pluginVersion: ZoteroGitSync.version,
				libraries: stats,
			}));
	...
	librarySlug(library) {
		...
		return `group-${library.groupID}-${ZoteroGitSync.Utils.sanitizeSegment(library.name, 40)}`;
	},


	itemPath(dir, key) {
		return `${dir}/items/${ZoteroGitSync.Utils.shard(key)}/${key}.json`;
	},
	...
	attachmentDir(key, lfs) {
		return `${lfs ? 'attachments-lfs' : 'attachments'}/${ZoteroGitSync.Utils.shard(key)}/${key}`;
	},
	...
	async _buildItemRecord({ item, ctx }) {
		...
		let markdownPath = `notes/${ZoteroGitSync.Utils.letterShard(markdownFilename)}/${markdownFilename}`;
		...
		for (let note of notes) {
			note.markdown = ZoteroGitSync.Utils.htmlToMarkdown(note.item.getNote(), { resolveImage });
		}
	...
	async _buildAttachment({ attachment, ctx, children, isTopLevel }) {
		...
		if (attachment.isLinkedFileAttachment()) {
			...
			let stat = path ? await ZoteroGitSync.Files.statFile(path) : null;
			if (!stat) {
			...
			sources = await ZoteroGitSync.Files.listDirectory(storageDir);
		...
		for (let source of sources) {
			if (config.maxAttachmentBytes && source.size > config.maxAttachmentBytes) {
				out.warnings.push(
					`Skipped ${label}/${source.relPath}: ${ZoteroGitSync.Utils.formatSize(source.size)} is over the configured limit`
				);
	...
	_markdownFilename(item) {
		let title = ZoteroGitSync.Utils.sanitizeSegment(item.getDisplayTitle() || 'Untitled', 80);
	...
	_buildItemMarkdown({ record, library }) {
		...
		let frontMatter = ZoteroGitSync.Utils.yamlFrontMatter({
			title: item.getDisplayTitle(),
	...
	_buildRootReadme(stats, config) {
		let lines = [
			'# Zotero Library',
			'',
			'This directory is written by the [Zotero Git Sync]'
			+ '(https://github.com/situkangsayur/zotero-generalgit-plugin) plugin. '
			+ 'Edits made here are not read back into Zotero unless you run **Import from Git** or accept them in a sync review.',
			'',
			'## Libraries',
			'',
		];
	...
	_textFile(text) {
		...
		return { bytes: ZoteroGitSync.Utils.encode(content) };
	},
