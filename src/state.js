/* Zotero Git Sync -- what this computer last synced, per repository
 *
...
ZoteroGitSync.State = {
	...
	/**
	 * @return {String} A file-system-safe name for this repository, branch and
	 * 		base path, readable enough to tell the files apart by hand
	 */
	id(config) {
		let key = [config.remoteURL, config.branch, config.basePath].join('\n');
		...
		let safe = s => ZoteroGitSync.Utils.sanitizeSegment(s || '-', 40).replace(/[\s@]+/g, '_');
		let label = ZoteroGitSync.Utils.remoteLabel(config.remoteURL).replace(/\//g, '__');
		return `${safe(label)}__${safe(config.branch)}__${hash.toString(16).padStart(8, '0')}`;
	},


	path(config) {
		return PathUtils.join(Zotero.Profile.dir, 'zotero-git-sync', 'state', `${this.id(config)}.json`);
	},
