...
This is the same format [Zotero GitHub Sync](https://github.com/situkangsayur/zotero-github-plugin)
writes — same layout, same files, same JSON — so a repository can move between the two
plugins (or be mirrored from one host to another) and keep syncing. Only the generated
`README.md` text and the `generator` field in the manifest differ. Each plugin keeps its
own record of what a computer last synced, so the first sync with the other plugin compares
cautiously, as on a new computer.

The repository is a mirror of the library, not the source of truth. Editing files here does
not change Zotero unless you run *Import from Git* or accept the change in a sync review, and the next sync overwrites anything
...
Git hosts' web interfaces slow down on directories with thousands of entries, so nothing that grows with the
library sits flat:
...
by default). Everything else is an ordinary Git blob — up to *Largest file to commit to Git
itself* (100 MB by default); a larger file with LFS off is skipped with a warning.
...
LFS objects count against the host's LFS storage limits, which differ per host and plan. LFS
objects are always uploaded before the commit that points at them is pushed; when the host
refuses an upload the sync fails with a message saying so rather than pushing pointers to
...
overwrites it, and *Import from Git* reads `items/**/*.json` — never the Markdown.
...
Content that already exists in the repository under another path is not stored or pushed
again (Git deduplicates by content), so a rename costs a tree update, not a re-upload.
