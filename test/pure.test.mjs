...
globalThis.ZoteroGitSync = { log(){}, warn(){}, logError: console.error, version: '0.1.0', getString: k => k };
for (const f of ['utils.js', 'exporter.js', 'importer.js', 'planner.js', 'sync.js']) vm.runInThisContext(readFileSync(`${SRC}/${f}`, 'utf8'));
const U = ZoteroGitSync.Utils, E = ZoteroGitSync.Exporter, I = ZoteroGitSync.Importer;

// base64 across block boundaries (HTTPS Basic credentials)
...
const token = new ZoteroGitSync.CancelToken();
const t0 = Date.now();
...
// remote URLs: labels, web pages, credentials kept out of messages
{
	const cases = [
		['git@gitlab.com:owner/repo.git', 'gitlab.com/owner/repo', 'https://gitlab.com/owner/repo'],
		['ssh://git@codeberg.org:2222/me/lib.git', 'codeberg.org/me/lib', 'https://codeberg.org/me/lib'],
		['https://user:tok@gitea.example.com:3000/me/lib.git', 'gitea.example.com/me/lib', 'https://gitea.example.com:3000/me/lib'],
		['https://bitbucket.org/ws/zot', 'bitbucket.org/ws/zot', 'https://bitbucket.org/ws/zot'],
		['http://git.lan/team/lib.git', 'git.lan/team/lib', 'http://git.lan/team/lib'],
		['/srv/git/zotero.git', 'git/zotero', ''],
		['file:///srv/git/zotero.git', 'git/zotero', ''],
		['C:\\repos\\zot.git', 'repos/zot', ''],
		['git@host:~/zotero.git', 'host/~/zotero', ''],
	];
	for (const [url, label, web] of cases) {
		assert.equal(U.remoteLabel(url), label, `label ${url}`);
		assert.equal(U.remoteWebURL(url), web, `web ${url}`);
	}
	assert.equal(U.parseRemoteURL('C:\\repos\\zot.git'), null, 'a Windows drive is not an scp host');
	assert.equal(U.redactURLs('fatal: unable to access https://user:secret@host/x.git/: 403'), 'fatal: unable to access https://host/x.git/: 403');
}

// -- Planner: three-way classification --------------------------------------
const P = ZoteroGitSync.Planner;
